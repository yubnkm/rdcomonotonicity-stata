...
Thus, the two conditional mean potential outcomes are comonotonic.
...
The command also constructs observation-level estimates of
...
as well as an estimated support indicator (S).

The example below uses five candidate bandwidths. With three or more values supplied to `bandwidth()`, `rdcomono` uses cross-validation to select the bandwidths.
...
When the `bootstrap()` option is supplied, `rdcomono` implements the multiplier bootstrap used by Deaner and Kwon (2025).

For bootstrap replication (b), independent standard exponential multipliers are generated,

[
e_{ib}\sim \operatorname{Exp}(1),
]

and the effective observation weights are

[
w_{ib}=w_i e_{ib}.
]

The model is then re-estimated using the bootstrap weights.
...
The first-stage bandwidths selected in the original fit and the nearest-neighbor frontier geometry are reused during the bootstrap.

`bootpoints(100)` requests 100 evaluation points for the (q_0) and (q_1) curves.

### Estimated potential-outcome mappings

`rdcomono_qplot` uses the bootstrap frame generated by `rdcomono` to produce three graphs:
...
Under comonotonicity, the two population mappings imply the same relationship between the treated and untreated conditional mean potential outcomes.

`rdcomono_qplot` therefore also plots (\widehat q_1) against the inverse relationship implied by (\widehat q_0).
...
Crosses indicate observations for which

[
S=1,
]

while circles indicate observations for which
...
Create the counterfactual treatment variable with:
...
`rdcomono_policy` estimates the mean outcome change generated by replacing the observed treatment rule with the counterfactual policy:
...
The estimated policy effect is averaged over all observations with `supported == 1`, rather than only over observations whose treatment status changes.
...
```stata
generate double true_counterfactual_mean = ///
    cond(D_counterfactual == 1, mu1, mu0)

generate double true_policy_change = ///
    true_counterfactual_mean - factual_mean ///
    if supported == 1

quietly summarize true_policy_change ///
    if supported == 1, meanonly
